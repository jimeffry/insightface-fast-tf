...
def parms():
    ...
    parser.add_argument('--file2-in',type=str,dest="file2_in",default="train2.txt",\
                        help='label files')
    parser.add_argument('--base-label',type=int,dest="base_label",default=None,\
                        help='label files')
...
def generate_list_from_dir(dirpath,out_file,label_num):
    ...
    file_name = []
    total_cnt = 0
    for idx,file_cnt in enumerate(files):
        '''
...
def f_w(file_out,cls_cnt,img_cnt):
    fw = open(file_out,'w')
    fw.write("{},{}".format(str(cls_cnt),str(img_cnt)))
    fw.close()
...
if __name__ == "__main__":
    ...
    if cmd_type == 'merge':
        ...
        cls_c,img_c = generate_list_from_dir(img_dir,out_file,label)
        #f_w(out_file,cls_c,img_c)
