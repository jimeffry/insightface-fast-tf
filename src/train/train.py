...
def train(args):
    ...
    logits = get_model(img_batch,cfgs.FEATURE_LEN)
    # ----------------------------------------------------------------------------------------------------build loss
    with tf.variable_scope('build_loss'):
        arc_logits = combine_loss(logits,label_batch,class_nums,margin_a,margin_m,margin_b,scale_s)
        ...
        cls_loss,soft_logits = entropy_loss(arc_logits,label_batch,class_nums)
        total_loss = cls_loss + weight_decay_loss
        acc_op,label_out,pred = cal_accuracy(soft_logits,label_batch)
    # ---------------------------------------------------------------------------------------------------add summary
                ...
                for epoch_tmp in range(epochs):
                    for step in range(np.ceil(train_img_nums/batch_size).astype(np.int32)):
                        ...
                        if step % cfgs.SHOW_TRAIN_INFO_INTE != 0 and step % cfgs.SMRY_ITER != 0:
                            ...
                            if step % cfgs.SHOW_TRAIN_INFO_INTE == 0 and step % cfgs.SMRY_ITER != 0:
                                start = time.time()
                                label_o,pred_out,global_stepnp, totalLoss,cls_l,acc,cur_lr = sess.run([label_out,pred,global_step, total_loss,cls_loss,acc_op,lr])
                                end = time.time()
                                print('label',label_o)
                                print('pred',pred_out)
